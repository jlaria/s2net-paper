...
df = results[,-3] %>% 
  filter(n_vars == 100, sigma2 == 2.5) %>%
  group_by(method, n_target, n_vars, scenario, sigma2) %>% summarise(Test = round(mean(test.loss),4)) %>% 
  unite(scenario, sigma2, scenario,  n_vars, n_target, sep = ",") %>% spread(key = c("scenario"), value = Test)
df

df.Box = results %>% select(N, method, n_target, n_vars, scenario, sigma2, test.loss) %>% 
  filter(n_target == 250, n_vars == 100, scenario == "unlucky", sigma2 == 2.5) %>% 
  spread(method, test.loss) %>% select(-N, -n_target, -n_vars, -scenario, -sigma2) %>% as.matrix()
