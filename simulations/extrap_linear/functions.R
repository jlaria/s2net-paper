...
generate_data = function(N, settings){
  for (j in 1:nrow(settings)) {
    ...
    for (n in 1:N) {
        train = simulate_extra(n_source, n_target, p = n_vars, shift = 1, scenario = scenario, response = "linear", sigma2 = sigma2)
        valid = simulate_extra(n_source = 2, n_target = 20, p = n_vars, shift = 1, scenario = scenario, response = "linear", sigma2 = sigma2)
        test = simulate_extra(n_source = 2, n_target = 100, p = n_vars, shift = 1, scenario = scenario, response = "linear", sigma2 = sigma2)
