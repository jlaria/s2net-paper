...
df = results[,-3] %>% group_by(method, n_target, n_vars) %>% summarise(Test = round(mean(test.loss), 2)) %>% 
  unite(scenario, n_target, n_vars, sep = ",") %>% spread(key = c("scenario"), value = Test)
df

df = results %>% group_by(method,n_source, n_target, n_vars) %>% summarise(Test = median(test.loss)) %>% 
  unite(scenario, n_source, n_target, n_vars, sep = ",") %>% spread(key = c("scenario"), value = Test)



df.Box = results %>% select(N, method, n_target, n_vars, test.loss) %>% 
  filter(n_target == 250, n_vars == 200) %>% spread(method, test.loss) %>% select(-N, -n_target, -n_vars) %>% as.matrix()
