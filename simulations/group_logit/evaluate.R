...
df = results[,-3] %>% group_by(method, n_target, n_vars) %>% summarise(Test = mean(test.auc)*100) %>% 
  unite(scenario, n_target, n_vars, sep = ",") %>% spread(key = c("scenario"), value = Test)
df

df.Box = results %>% select(N, method, n_target, n_vars, test.auc) %>% 
  filter(n_target == 250, n_vars == 200) %>% spread(method, test.auc) %>% select(-N, -n_target, -n_vars) %>% as.matrix()
